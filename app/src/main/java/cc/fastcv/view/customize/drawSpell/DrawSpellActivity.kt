package cc.fastcv.view.customize.drawSpell
...
import cc.fastcv.view.customize.drawSpell.custom.DrawSpellView
...
class DrawSpellActivity : AppCompatActivity(), TextWatcher, SeekBar.OnSeekBarChangeListener {
    ...
    private fun updateIvSelect(index:Int) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            when(index) {
                0 -> {
                    findViewById<ImageView>(R.id.iv_16).foreground = getDrawable(R.drawable.shape_bitmap_selected_bg)
                }
                1 -> {
                    findViewById<ImageView>(R.id.iv_32).foreground = getDrawable(R.drawable.shape_bitmap_selected_bg)
                }
                2 -> {
                    findViewById<ImageView>(R.id.iv_64).foreground = getDrawable(R.drawable.shape_bitmap_selected_bg)
                }
                3 -> {
                    findViewById<ImageView>(R.id.iv_128).foreground = getDrawable(R.drawable.shape_bitmap_selected_bg)
                }
                else -> {}
            }
            ivIndex = index
