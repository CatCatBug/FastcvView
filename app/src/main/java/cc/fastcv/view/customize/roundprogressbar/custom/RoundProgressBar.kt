...
class RoundProgressBar : View {
    ...
    override fun draw(canvas: Canvas?) {
        ...
        canvas?.drawLine(mViewHeight/2,mViewHeight/2,mViewWidth - mViewHeight/2,mViewHeight/2,mPaint)

        if (mProgress != 0f) {
            mPaint.color = Color.BLUE
            canvas?.drawLine(mViewHeight/2,mViewHeight/2,mViewHeight/2 + (mViewWidth - mViewHeight) * (mProgress / 100.0f), mViewHeight/2,mPaint)
        }
    }
}